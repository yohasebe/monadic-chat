- [April, 2026] 1.0.0-beta.16
  ...
  - **Privacy Filter — English-only by design (Install Options UI cleanup)**: The "Additional languages" section in Settings → Install Options has been removed. No built-in app declares non-English `languages` in its MDSL, so the spaCy multilingual NER models would have been installed without ever being exercised. The `PRIVACY_LANGS` env value is pinned to `en` whenever Settings are saved, and the rebuild-needed badge no longer triggers on language changes. Power users who maintain their own MDSL files can still override `PRIVACY_LANGS` directly via the env file. Existing users who previously rebuilt the privacy container with `PRIVACY_LANGS=en,ja,...` keep the unused language models on disk until the next manual rebuild — there is no functional impact, just ~150 MB per language of unused image weight.
