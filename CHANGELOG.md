- [April, 2026] 1.0.0-beta.16
  ...
  - **Privacy Filter — installed by default**: The Privacy Filter container is now part of the default build set, so any Privacy-aware app (Chat, Mail Composer, Chat Plus, Translate, Second Opinion) can be toggled on without a separate install step. The "Privacy Filter" section in Settings → Install Options has been removed; runtime opt-out is still available by setting `PRIVACY_FILTER=false` in `~/monadic/config/env`. `PRIVACY_LANGS` is pinned to `en` (Privacy Filter is English-only by design). The "Build Privacy" button in Settings → Actions remains for manual rebuilds.
