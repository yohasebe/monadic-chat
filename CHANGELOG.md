- [May, 2026] 1.0.0-beta.17
  ...
  - **Unblocks Veo and Grok Imagine image-to-video**: The Gemini Veo and xAI Grok Imagine video-generator MDSLs carried an `INITIAL GREETING (IMPORTANT - First message only)` block ending with `CRITICAL: The first message must be a plain text greeting. Do NOT output JSON, do NOT call tools` that the model was applying to **every** turn — when a user uploaded an image plus a video description, the model would silently re-emit the greeting template (in the wrong language, ignoring tool calls and the Workflow section) instead of calling `generate_video_with_veo` / `generate_video_with_grok_imagine`. Image-to-video had not worked end-to-end on these providers since the MDSLs were introduced. The greeting block is now a conditional one-liner ("only if this is the very first turn AND the user has not yet submitted a request"), letting the model call the appropriate tool on actual user input. OpenAI Sora's MDSL never had the strict CRITICAL clause and is unchanged.
