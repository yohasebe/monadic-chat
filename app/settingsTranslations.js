  ...
  en: {
    settings: {
      ...
      labels: {
        ...
        installPrivacyTitle: "Privacy Filter — Additional Languages",
        installPrivacyDesc: "English is always enabled. Enable additional languages to mask PII in non-English content. All language models are preinstalled in the Privacy container; changes apply when you save — no rebuild needed.",
  ...
  ja: {
    settings: {
      ...
      labels: {
        ...
        installPrivacyTitle: "プライバシーフィルタ — 追加言語",
        installPrivacyDesc: "英語は常に有効です。英語以外のコンテンツで個人情報をマスクするには追加言語を有効化してください。全言語のモデルはPrivacyコンテナにプリインストール済みで、変更は保存時に反映されます（リビルド不要）。",
  ...
  zh: {
    settings: {
      ...
      labels: {
        ...
        installPrivacyTitle: "隐私过滤器 — 附加语言",
        installPrivacyDesc: "英语始终启用。启用附加语言以屏蔽非英语内容中的个人信息。所有语言模型已预装在隐私容器中；更改在保存时生效，无需重新构建。",
  ...
  ko: {
    settings: {
      ...
      labels: {
        ...
        installPrivacyTitle: "프라이버시 필터 — 추가 언어",
        installPrivacyDesc: "영어는 항상 활성화됩니다. 영어 이외의 콘텐츠에서 개인정보를 마스킹하려면 추가 언어를 활성화하세요. 모든 언어 모델은 Privacy 컨테이너에 사전 설치되어 있으며 변경 사항은 저장 시 적용됩니다(재빌드 불필요).",
  ...
  es: {
    settings: {
      ...
      labels: {
        ...
        installPrivacyTitle: "Filtro de Privacidad — Idiomas Adicionales",
        installPrivacyDesc: "El inglés siempre está habilitado. Active idiomas adicionales para enmascarar información personal en contenido no inglés. Todos los modelos de idioma están preinstalados en el contenedor de Privacidad; los cambios se aplican al guardar, sin recompilación.",
  ...
  fr: {
    settings: {
      ...
      labels: {
        ...
        installPrivacyTitle: "Filtre de confidentialité — Langues supplémentaires",
        installPrivacyDesc: "L'anglais est toujours activé. Activez des langues supplémentaires pour masquer les informations personnelles dans le contenu non anglais. Tous les modèles de langue sont préinstallés dans le conteneur Privacy ; les modifications s'appliquent à l'enregistrement, sans reconstruction.",
  ...
  de: {
    settings: {
      ...
      labels: {
        ...
        installPrivacyTitle: "Privacy-Filter — Zusätzliche Sprachen",
        installPrivacyDesc: "Englisch ist immer aktiviert. Aktivieren Sie weitere Sprachen, um persönliche Daten in nicht-englischen Inhalten zu maskieren. Alle Sprachmodelle sind im Privacy-Container vorinstalliert; Änderungen werden beim Speichern wirksam, ohne Neuaufbau.",
        installExtractorTitle: "Knowledge Base Quality Pack (Docling)",
