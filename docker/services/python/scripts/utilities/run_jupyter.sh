...
start_jupyterlab() {
    ...
    jupyter lab --LabApp.default_url='/lab/tree' --core-mode --ip='*' --port=8889 --allow-root --no-browser --NotebookApp.token='' --NotebookApp.password='' --NotebookApp.terminado_settings='{"shell_command": ["/bin/bash"]}' > "$SCRIPT_DIR/jupyter_lab.log" 2>&1 &
