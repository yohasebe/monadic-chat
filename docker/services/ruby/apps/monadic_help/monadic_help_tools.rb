...
require_relative '../../lib/monadic/help'
...
module MonadicHelpTools
  def find_help_topics(text:, top_n: 10, chunks_per_result: nil, include_internal: nil)
    with_help_database do |db|
      ...
      results = db.find_closest_text_multi(text,
  ...
  def get_help_document(doc_id:, include_internal: nil)
    with_help_database do |db|
      include_internal = (ENV['DEBUG_MODE'] == 'true') if include_internal.nil?
      snippets = db.get_text_snippets(doc_id, include_internal: include_internal)
  ...
  def list_help_sections(language: nil, include_internal: nil)
    with_help_database do |db|
      include_internal = (ENV['DEBUG_MODE'] == 'true') if include_internal.nil?
      titles = db.list_titles(language: language, include_internal: include_internal)
  ...
  def search_help_by_section(text:, section:, top_n: 3, chunks_per_result: nil, include_internal: nil)
    with_help_database do |db|
      ...
      all_results = db.find_closest_text(text, top_n: top_n * 10, include_internal: include_internal)
  ...
  def with_help_database
    Monadic::Help.installation.with_search { |db| yield db }
  rescue Monadic::Help::Installation::Unavailable => e
    state = e.status[:state]
    message = case state
              when 'installing'
                'Help data is being installed. Open Help Data to check progress.'
              when 'unavailable'
                'The Help data service is unavailable. Open Help Data and try again.'
              else
                'Help data is not ready. Open Help Data from Monadic Chat Info and install or reinstall it.'
              end
    { error: message, code: 'help_database_unavailable', state: state,
      action: { label: 'Help Data', url: '/help/database' } }
  rescue StandardError
    { error: 'Help search is unavailable. Open Help Data to check its status.',
      code: 'help_search_failed', action: { label: 'Help Data', url: '/help/database' } }
  end
end
