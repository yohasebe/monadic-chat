module VisualWebExplorerTools
  ICON = '<i class="fa-solid fa-globe-magnifying-glass"></i>'
  
  DESCRIPTION = <<~HTML
    This app helps you find and capture web pages as screenshots or extract their text content in Markdown format.
    Perfect for research, documentation, archiving web content, or analyzing page content.
    <br><br>
    <b>Features:</b>
    <ul>
      <li>Web search integration to find relevant sites on any topic</li>
      <li>Capture full web pages as multiple viewport-sized images</li>
      <li>Extract text content in clean Markdown format</li>
      <li>Automatic fallback to image recognition when text extraction fails</li>
      <li>Customizable viewport dimensions for screenshots</li>
  ...
  INITIAL_PROMPT = <<~TEXT
    Welcome to Visual Web Explorer! 📸

    I can help you find and capture web pages as screenshots or extract their text content in Markdown format.

    **What I can do:**
    1. **Web Search & Capture**: Find relevant websites and capture them
       • Search for information on any topic
       • Present relevant URLs for your approval
       • Capture selected pages automatically
    
    2. **Screenshot Mode**: Capture pages as viewport-sized images
       • Multiple screenshots with automatic scrolling
       • Customizable viewport dimensions
       • Desktop, tablet, mobile, and print presets
       • Overlap between images for seamless reading
    
    3. **Text Extraction Mode**: Convert web content to Markdown
       • Clean, readable Markdown format
       • Preserves headings, lists, and links
       • Extracts metadata (title, description, etc.)
       • Automatic fallback to image recognition if needed

    **Example requests:**
    • "Find information about renewable energy" → I'll search and show relevant sites
    • "Research best practices for web development" → I'll find and capture useful resources
    • "Capture screenshots of https://github.com" → Direct URL capture
    • "Extract text from https://example.com" → Convert to Markdown
    • "Find and capture mobile-friendly cooking recipe sites" → Search + mobile capture

    Ask me anything or provide a URL to get started! 🚀
  TEXT
  ...
  SYSTEM_PROMPT = <<~TEXT
    ...
    Key behaviors:
    1. Default to screenshots for general capture requests
    2. Use text extraction when users mention "text", "content", "markdown", or "extract"
    3. When text extraction fails or returns minimal content, automatically suggest using image recognition
    4. For screenshots, use default values (1920x1080 viewport, 100px overlap) unless specified
    5. Explain options clearly and provide results in a user-friendly format
