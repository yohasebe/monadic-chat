...
module Monadic
  module Library
    ...
    #   2. Search library_summaries (filtered by app scope) for the
    #      top-K most relevant conversations.
    #   3. For each candidate conversation, search library_turns scoped to
    #      that conversation_id for the most relevant turn-level passages.
    ...
    # placeholders — the second pass at turn level dominates the ranking.
    module Retriever
      module_function

      DEFAULT_TOP_N = 3
      # summary_top_k must be generous enough that small corpora
      # (10-50 entries) don't lose user-saved conversations during the
      # summary cascade. Placeholder summaries make short threads score
      # lower than long documents, so a tight top_k systematically filters
      # them out at the summary stage. 20 covers a typical individual
      # library; the final `first(top_n)` cut still trims down to the
      # requested size for larger corpora.
      DEFAULT_SUMMARY_TOP_K = 20
      DEFAULT_TURNS_PER_CONV = 3

      # @param query [String] user query
      # @param store [Monadic::Library::Store]
      # @param app_name [String, nil] app class name of the requester.
      #   Restricts results to entries scoped to that app or "Global".
      #   Pass nil to search the full library (KB UI surfaces).
      ...
      def cascade_search(query,
        ...
        all_turn_hits = summary_hits.flat_map do |sh|
          ...
          # Turn-level points carry only conversation_id / scope_app /
          # turn_idx — fields like `source` and `content_type` live on
          # the summary payload only. Forwarding payload_filter here
          # would zero-out narrowed queries; the summary cascade has
          # already enforced source / content_type transitively, so any
          # conv_id we see here is already in scope.
