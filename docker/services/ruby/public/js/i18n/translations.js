...
const webUITranslations = {
  en: {
    ui: {
      ...
      libPrivacyNote: "Privacy Filter is active. Saving stores the restored (unmasked) conversation text on local disk. The Knowledge Base is unencrypted; treat the saved entry as sensitive and avoid Document DB exports unless the recipient is trusted.",
      libAnonymizeLabel: "Anonymize before saving",
      libAnonymizeDesc: "Replace tracked PII (names, emails, phone numbers, etc.) with placeholders before storing. Recommended when Privacy Filter is on. Note: anonymized entries cannot be restored later — the registry is per-session and is not saved.",
      libBadgeAnonymized: "Anonymized — placeholders only, no original PII on disk",
      libBadgePlainWithPrivacy: "Privacy was on but this entry is stored with original PII",
  ...
  ja: {
    ui: {
      ...
      libPrivacyNote: "Privacy Filter が有効です。保存すると、画面に表示されている復元 (マスク解除) 後の本文がローカルディスクに保管されます。Knowledge Base は暗号化されないため、保存内容は機微情報として扱い、ドキュメント DB のエクスポート先は信頼できる相手に限定してください。",
      libAnonymizeLabel: "保存前に匿名化する",
      libAnonymizeDesc: "登録済みの個人情報 (名前・メール・電話番号など) を保存前にプレースホルダーに置換します。Privacy Filter が有効な場合は推奨。注: 匿名化されたエントリは後から復元できません — registry はセッション単位で保存対象外です。",
      libBadgeAnonymized: "匿名化済み — ディスクには placeholder のみで原文 PII は残っていません",
      libBadgePlainWithPrivacy: "Privacy Filter が有効なまま保存されたため、原文 PII がディスクに残っています",
  ...
  zh: {
    ui: {
      ...
      libPrivacyNote: "隐私过滤器已启用 — 保存的文本是对话中显示的还原（未掩码）版本。",
      libSaveButton: "保存",
      libEmpty: "知识库为空。",
  ...
  ko: {
    ui: {
      ...
      libPrivacyNote: "개인 정보 필터가 켜져 있습니다 — 저장되는 본문은 대화창에 표시된 복원 (마스킹 해제) 후의 텍스트입니다.",
      libSaveButton: "저장",
  ...
  es: {
    ui: {
      ...
      libPrivacyNote: "El filtro de privacidad está activo — el texto guardado es la versión restaurada (sin enmascarar) visible en el diálogo.",
  ...
  fr: {
    ui: {
      ...
      libPrivacyNote: "Le filtre de confidentialité est activé — le texte enregistré est la version restaurée (non masquée) affichée dans le dialogue.",
  ...
  de: {
    ui: {
      ...
      libPrivacyNote: "Privacy Filter ist aktiv — der gespeicherte Text ist die wiederhergestellte (entmaskierte) Version, die im Dialog sichtbar ist.",
