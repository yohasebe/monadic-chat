...
const webUITranslations = {
  en: {
    ui: {
      ...
      resetDescription: "Press \"Reset\" to clear conversation while keeping the current app selection. Click the logo in the top left to clear conversation and return to the default app.",
  ...
  ja: {
    ui: {
      ...
      resetDescription: "「リセット」を押すと、現在のアプリ選択を維持したまま会話をクリアします。左上のロゴをクリックすると会話をクリアしてデフォルトのアプリに戻ります。",
  ...
  zh: {
    ui: {
      ...
      resetDescription: "按\"重置\"清除对话，同时保持当前的应用程序选择。点击左上角的徽标可清除对话并返回默认应用程序。",
  ...
  ko: {
    ui: {
      ...
      resetDescription: "\"재설정\"을 누르면 현재 앱 선택을 유지하면서 대화를 지웁니다. 왼쪽 상단의 로고를 클릭하면 대화를 지우고 기본 앱으로 돌아갑니다.",
  ...
  es: {
    ui: {
      ...
      resetDescription: "Presione \"Reiniciar\" para limpiar la conversación mientras mantiene la selección actual de la aplicación. Haga clic en el logo en la parte superior izquierda para limpiar la conversación y volver a la aplicación predeterminada.",
  ...
  fr: {
    ui: {
      ...
      resetDescription: "Appuyez sur \"Réinitialiser\" pour effacer la conversation tout en conservant la sélection actuelle de l'application. Cliquez sur le logo en haut à gauche pour effacer la conversation et revenir à l'application par défaut.",
  ...
  de: {
    ui: {
      ...
      resetDescription: "Drücken Sie \"Zurücksetzen\", um die Konversation zu löschen und dabei die aktuelle App-Auswahl beizubehalten. Klicken Sie auf das Logo oben links, um die Konversation zu löschen und zur Standard-App zurückzukehren.",
