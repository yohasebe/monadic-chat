...
if options[:check]
  api_key_status = get_env_var("GROQ_API_KEY") != false
  model_name = get_env_var("GROQ_MODEL") || "false"
  puts({"API_KEY" => api_key_status, "MODEL" => model_name}.to_json)
  exit
end

def query(message: "", model: "llama2-70b-4096")
  num_retrial = 0
  begin
    if File.file?("/.dockerenv")
      api_key = File.read("/monadic/data/.env").split("\n").find do |line|
        line.start_with?("GROQ_API_KEY")
      end.split("=").last
    else
  ...
  target_uri = "#{API_ENDPOINT}/v1/chat/completions"
