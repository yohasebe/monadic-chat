...
RSpec.describe "DeepSeek Reasoning Content Extraction" do
  ...
  describe "Final response structure" do
    it "includes reasoning in message when reasoning_content available" do
      result = {
        "choices" => [
          {
            "message" => {
      ...
      expect(result["choices"][0]["message"]["reasoning"]).to eq("Step 1: Analyze\n\nStep 2: Conclude")
      expect(result["choices"][0]["message"]["content"]).to eq("The answer is 42")
    end

    it "does not include reasoning field when reasoning_content empty" do
      result = {
        "choices" => [
          {
            "message" => {
      ...
      expect(result["choices"][0]["message"].key?("reasoning")).to be false
    end

    it "does not include reasoning field when reasoning_content nil" do
      result = {
        "choices" => [
          {
            "message" => {
  ...
  describe "Model compatibility" do
    it "supports DeepSeek reasoning models" do
      models = ["deepseek-reasoner", "deepseek-r1"]

      models.each do |model_name|
        is_reasoning = model_name.include?("reasoner") || model_name.include?("-r1")
        expect(is_reasoning).to be true
      end
    end

    it "identifies non-reasoning DeepSeek models" do
  ...
  describe "Streaming behavior" do
    it "sends reasoning fragments in real-time" do
      fragments = []

      # Simulate streaming deltas
      deltas = [
        { "choices" => [{ "delta" => { "reasoning_content" => "Analyzing " } }] },
        { "choices" => [{ "delta" => { "reasoning_content" => "the problem " } }] },
        { "choices" => [{ "delta" => { "reasoning_content" => "carefully" } }] }
      ]

      deltas.each do |json|
        reasoning = json.dig("choices", 0, "delta", "reasoning_content")
        fragments << reasoning unless reasoning.to_s.strip.empty?
      end

      expect(fragments).to eq(["Analyzing ", "the problem ", "carefully"])
      expect(fragments.join("")).to eq("Analyzing the problem carefully")
    end
  end
end
