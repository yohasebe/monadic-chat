...
RSpec.describe Monadic::Utils::ModelSpec do
  ...
  describe 'xAI provider defaults' do
    it 'resolves grok-4.7 as the chat and vision default' do
      expect(described_class.get_provider_default('xai', 'chat')).to eq('grok-4.7')
      expect(described_class.default_vision_model('xai')).to eq('grok-4.7')
    end
    ...
    it 'exposes the selectable image models with the cheapest as default' do
      ...
      expect(models).to eq(%w[grok-imagine-image grok-imagine-image-2.0])
      expect(described_class.default_image_model('xai')).to eq('grok-imagine-image')
    end

    it 'catalogs both current Grok chat models as vision-capable' do
      %w[grok-4.7 grok-4.6].each do |model|
        expect(described_class.get_model_property(model, 'vision_capability')).to be true
      end
    end
  end
end
