...
Monadic Chatは、複数のブラウザタブを同時に開くことをサポートし、インテリジェントなセッション共有を提供します：

**セッション分離：**
- 各ブラウザセッション（ブラウザ + プロファイル + モード）は、クッキーに保存される一意のセッションIDを持ちます
- 同じブラウザセッション内のすべてのタブは、同じ会話状態を共有します
- 異なるブラウザ、ブラウザプロファイル、またはシークレット/プライベートモードは、別個のセッションを作成します
- サーバーモードでは、各ユーザーのセッションは他のユーザーから完全に分離されます

**セッション共有の例：**
- ✅ 同じブラウザ、複数のタブ → **セッション共有**（変更がタブ間で同期）
- ✅ 同じブラウザ、複数のウィンドウ → **セッション共有**
- ❌ Chrome + Firefox（同じ端末） → **別個のセッション**
- ❌ 通常モード + シークレットモード → **別個のセッション**
- ❌ 異なる端末（PC + スマートフォン） → **別個のセッション**
- ❌ 異なるブラウザプロファイル → **別個のセッション**

**同期されるデータ：**
- アクティブなアプリの選択（新しいタブは現在のアプリを自動的に選択）
- モデルとパラメータ設定（StartまたはSendボタン経由で送信時）
- 会話メッセージと編集
- メッセージの削除と変更

**注意：** UI上でのパラメータ変更は、StartまたはSendをクリックするまでローカルのままです。クリック後、同じセッション内のすべてのタブに同期されます。
