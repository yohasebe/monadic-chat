...
**音声と速度**: アシスタントの声はアプリのコントロール行にあるセレクターで選択します（音声設定パネルのTTS設定はここでは使われません）。選択肢の数は OpenAI が10（既定: alloy）、xAI が26（既定: eve）、Gemini が30（既定: Kore）です。再生速度のスライダー（0.25〜1.5）は OpenAI のみ利用できます。音声の選択はプロバイダーごとに記憶されます。

**ターン判定**（OpenAI 変種のみ）: 発話の終わりの判定方式を選べます。既定は無音で判定します。「意味で判定」モード（ゆっくり・標準・すばやく）は、話の内容が完結したと聞こえるまでモデルに待たせる方式です。どの程度違いが出るかは言語や話し方によって変わるので、両方試してみてください。選択は記憶され、次のセッションから適用されます。
...
ライブ表示では、アシスタントが今読み上げている文がハイライトされ、声を文字で追えます。テキストは音声よりかなり早く出そろうため、長い応答ほど役に立ちます。ハイライトは音声そのものに追従し、ツールの実行中は止まり、割り込んだ場合は発話が止まった位置で止まります。カード表示では出ません。
...
**料金**: 接続中は無音の時間も課金対象です。単価はプロバイダーによってかなり異なるため、長いセッションの前に各プロバイダーの料金ページを確認してください。実使用量にもとづく料金の目安は画面上部に表示されます。この表示は Live Conversation 固有です。誰も話していない間も課金が進む唯一のアプリであるため、あとから気づくのではなく常に合計が見える形にしています。
