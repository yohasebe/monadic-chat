...
**音声と速度**: アシスタントの声はアプリのコントロール行にあるセレクターで選択します（音声設定パネルのTTS設定はここでは使われません）。選択肢の数は OpenAI が10（既定: alloy）、xAI が26（既定: eve）、Gemini が30（既定: Kore）です。再生速度のスライダー（0.25〜1.5）は OpenAI のみ利用できます。選んだ声はプロバイダーごとに記憶され、次にアプリを開いたときに復元されます。
