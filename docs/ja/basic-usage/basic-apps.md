...
Webページのスクリーンショット撮影や、AIによるインタラクティブなブラウザ操作を行うアプリケーションです。静的なスクリーンショット撮影とリアルタイムのブラウザ操作の2つのモードに対応しています。

**主な機能:**
- **スクリーンショットモード**: ページ全体を自動スクロールしながら複数のビューポートサイズの画像として撮影
- **インタラクティブブラウザモード**: AIが操作する実際のChromeブラウザを起動し、リンクのクリック、フォーム入力、スクロールなどを実行。noVNCウィンドウでブラウザの動作をリアルタイムに確認可能
...
インタラクティブブラウザモードでは、AIがSeleniumコンテナ内の実際のChromeブラウザを操作します。noVNCウィンドウを通じてブラウザの動作をリアルタイムで確認できます:

- **Electronアプリ**: AIがブラウザセッションを開始すると、noVNCウィンドウが自動的に開きます。メニューバーの **Open > noVNC を開く** から手動で開くこともできます
- **開発モード**: ブラウザの別タブで `http://localhost:7900` を開いてください

AIはリンクのクリック、テキスト入力、ページのスクロール、ページ間の移動など、1セッションあたり最大20アクションを実行できます。

**使用例:**
- `"https://github.com のスクリーンショットを撮って"` - 複数のスクリーンショットを撮影
- `"https://example.com をブラウザで開いてAboutリンクをクリックして"` - インタラクティブ操作
