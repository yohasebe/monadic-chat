...
**Google Gemini** は品質と速度のトレードオフが異なる複数のImagenモデルを提供：
- **Imagen 4 Fast**（デフォルト）：最速で良好な品質
- **Imagen 4**：標準モデル、品質と速度のバランス
- **Imagen 4 Ultra**：最高品質、生成速度は遅い
- **Imagen 3**：レガシーモデル（引き続きサポート）
- **Gemini 2.5 Flash**：編集に対応した多用途マルチモーダルモデル

**OpenAI** はDALL-EとGPT-Imageモデルをサポートし、透明な背景、複数の出力形式、高解像度生成などの機能を提供。

**Grok/xAI** は自動プロンプト改善機能を持つ画像生成モデルを使用。
...
#### OpenAI (Sora 2)
対応モデル:
- `sora-2` — 高速モデル（反復作業やプロトタイピング向け、デフォルト）
- `sora-2-pro` — 高品質モデル（本番用、処理時間長め、コスト高）

**モデル選択ポリシー:**
- デフォルトでは常に `sora-2` を使用（コスト効率良し、高速）
- `sora-2-pro` は以下のキーワードで明示的に要求された場合のみ使用:
  - "高品質", "本番用", "プロ", "最終版", "公開用"（日本語）
  - "high quality", "production", "professional", "pro", "final version", "publish"（英語）
- キーワードなしでもプロフェッショナルな用途と判断された場合、アシスタントが確認してから `sora-2-pro` を選択

**仕様:**
- 対応サイズ: 1280x720, 1920x1080, 1080x1920（縦向き）, 720x1280（縦向き）
- 長さ: 4秒、8秒、16秒
- デフォルト: 1280x720、8秒、sora-2モデル

**コンテンツ制限:**
- 18歳未満向けのコンテンツのみ
- 著作権のあるキャラクターや音楽は使用不可
- 実在の人物（著名人を含む）は生成不可
- 人の顔が含まれる画像は使用不可
...
3. Remix（OpenAIのみ）: 動画生成後、修正を依頼
4. 必要に応じて品質の設定を指定（OpenAI: キーワード使用、Gemini: 高品質モデルを依頼）

?> **注意:** 動画生成は通常2～6分（OpenAI）または11秒～6分（Gemini）かかります。生成された動画は「共有フォルダ」に保存され、チャットインターフェースに表示されます。

**使用例：**
- 「山々に沈む夕日の動画を作成して」 → sora-2でテキストから動画生成
- 「高品質なマーケティング動画を作成」 → sora-2-proでテキストから動画生成
