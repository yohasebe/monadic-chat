...
- **RAG オプトイン (セッション単位)** — 任意のチャットセッションで **Use Knowledge Base for retrieval** トグルを ON にすると、LLM が応答中に `library_search` を呼び出せます。検索カスケードは現在アクティブなアプリのスコープフィルター (`scope_app IN [current_app, "Global"]`) を適用します。デフォルト OFF、最初のメッセージ送信でセッション中はロックされます。
