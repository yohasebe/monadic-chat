...
**Google Gemini** は以下をサポート：
- Imagen 4 Fast（デフォルト）、Imagen 4、Imagen 4 Ultra、Imagen 3
- Gemini 2.5 Flash（画像編集用）

**OpenAI** はDALL-EおよびGPT-Imageモデルをサポート。

**Grok/xAI** は独自の画像生成モデルを使用。

Image Generatorの対応プロバイダーは冒頭の表を参照してください。モデルの機能や違いについては、各プロバイダーの公式ドキュメントを参照してください。
