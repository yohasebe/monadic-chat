...
xAI Grokバリアントを選択している間は、音声入力と音声出力が自動的にxAIのSpeech-to-Text / Text-to-Speechエンジンを使用します（音声設定パネルの対応ドロップダウンはその間ロックされます）。アシスタントの発話には自然な表現（短い間、笑い、ささやくような一言など）が含まれることがあります。これらは音声にのみ反映され、チャット履歴のテキストには表示されません。
