...
**Export as PDF**<br />
`Export as PDF`ボタンをクリックすると、ブラウザの印刷ダイアログが開き、現在の会話をPDFファイルとして保存できます。PDFエクスポートには、すべてのメッセージ、シンタックスハイライト付きのコードブロック、数式（MathJaxでレンダリング）、および画像が含まれます。出力はA4用紙サイズに合わせてフォーマットされ、適切なページネーションが施されるため、印刷時やPDF保存時に読みやすくなっています。
