...
**PDF出力**<br />
`PDF出力`ボタンをクリックすると、ブラウザの印刷ダイアログが開き、現在の会話をPDFファイルとして保存できます。PDFエクスポートには、すべてのメッセージ、シンタックスハイライト付きのコードブロック、数式（MathJaxでレンダリング）、および画像が含まれます。出力はA4用紙サイズに合わせてフォーマットされ、適切なページネーションが施されるため、印刷時やPDF保存時に読みやすくなっています。
...
- **ElevenLabs (v3 Alpha)** — ELEVENLABS_API_KEY が設定されている場合、最新の `eleven_v3` モデルも選択できます。非常に表現力豊かな音声が得られますがアルファ版のためレイテンシーが高く、1リクエストあたり3,000文字までなど制約があります。生成結果を確認したうえで利用してください。また生成処理が完了してからまとめて音声が返る仕組みのため、ストリーミング再生は行われず、音声の再生開始までしばらく待機します。
