...
デスクトップ環境のWebブラウザからの利用に加えて、Monadic ChatはサーバーモードでノートPC、タブレット、スマートフォンなど様々なデバイスのWebブラウザからアクセスすることができます。サーバーモードで実行している場合、Web UIはレスポンシブであり、異なる画面サイズに適応するため、小さなスマートフォン画面でも使用可能です（ただし、Jupyter機能などの一部の機能は利用できません）。レスポンシブデザインは画面幅が767px以下の場合に自動的にモバイル最適化レイアウトに切り替わります。

### デスクトップアプリのボタン

内部ブラウザモードで Electron デスクトップアプリを実行すると、Web UI の右下に以下の 4 つのボタンが表示されます。

- Zoom In：ページのズームインを行います。
- Zoom Out：ページのズームアウトを行います。
- UI Reset：セッションデータをクリアし、UI をリロードします。
- Focus Main Window：メインウィンドウを前面に表示します。
