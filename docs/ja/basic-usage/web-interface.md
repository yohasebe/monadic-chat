...
サーバーモードでは、ループバック以外（LAN クライアント）からのアクセスに認証が必須です。モードを初めて有効化すると、Monadic Chat が `~/monadic/config/env` にランダムな 256-bit トークン（`MONADIC_AUTH_TOKEN`）を生成し、ホストコンソールには完全な共有可能 URL（例: `http://192.168.1.50:4567/?monadic_auth=<token>`）が表示されます。LAN 上の端末でその URL を開けば最初のリクエストで認証され、トークンは Cookie として保存されるため、以降の遷移では URL にトークンを含める必要がなくなります。ホストマシンからのループバック（127.0.0.1）リクエストは認証をスキップするため、ローカルの Electron ウェブビューは常に動作します。トークンをローテーションする場合は env ファイルから `MONADIC_AUTH_TOKEN` を削除して Monadic Chat を再起動してください。
...
**Import**<br />
外部JSONファイルから会話データを読み込みます。保存されたセッションが非推奨（deprecated）モデルを使用している場合、推奨される後継モデルに自動的に置き換えられ、通知が表示されます。
...
音声合成のプロバイダーを選択します（OpenAI、ElevenLabs、Gemini、Mistral、xAI Grok、Web Speech API）。Auto Speechを有効にし、音声マーカーを解釈できるプロバイダー（xAI Grok / ElevenLabs v3 / Gemini TTS）を選ぶと、このドロップダウンの下に✨ **Expressive Speech** バッジが表示されます。バッジが表示されている間、アシスタントの応答には自然な表現（間、笑い、ささやきなど）が含まれることがあります。これらは音声合成エンジンが音声として解釈するもので、チャット履歴のテキストには表示されません。
...
!> **ローカルストレージ（Qdrant + multilingual-e5-base）を使用している場合**: Monadic Chatをアップデートしたり、コンテナを再構築したりする際にはデータベースが破棄される可能性がありますので、後でリストアするために`Export Document DB`機能でデータをエクスポートしてください。詳細は[PDFストレージモード](./pdf_storage.md)を参照してください。
