...
### Q: 複数のブラウザタブで異なる会話を使用できますか？ :id=multiple-tabs

**A**: はい！各ブラウザタブは完全に独立した状態を維持します：
- タブごとに異なる会話
- タブごとに異なるアプリとパラメータ
- タブをリフレッシュしてもそのタブのセッションが保持される
- タブを閉じるとそのタブのセッションのみがクリアされる

これにより、例えば1つのタブでChatを使用しながら、別のタブでCode Interpreterを実行するなど、複数のタスクを同時に進めることができます。
