...
### Q: 読み込み中のスピナーが早く消えてしまうのはなぜですか？ :id=spinner-disappearing

**A**: これは一部のプロバイダー（DeepSeek、Perplexity、Ollama）で発生していた既知の問題ですが、修正されました。まだこの問題が発生する場合：
...
2. **ブラウザキャッシュをクリア**
   - ハードリフレッシュ（Ctrl+F5またはCmd+Shift+R）
   - 必要に応じてサイトデータをクリア

3. **プロバイダーの状態を確認**
   - 一部のプロバイダーで一時的な問題がある可能性
   - テストのためプロバイダーを切り替えてみる
