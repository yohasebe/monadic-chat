...
`Coding Assistant`アプリは、様々なプログラム（Python, Ruby, JavaScript, etc.）の作成を支援するための機能を提供します。AIエージェントにコードを実行させることはできませんが、ソースコードを提供して、問題点の修正を依頼したり、機能の追加を依頼したりすることができます。このアプリはプロンプトキャッシング（Anthropic Claude）をサポートしており、効率的な繰り返し修正が可能です。
...
2. 必要なポート（4567、3100、8889、4444、5900、7900）が他のアプリケーションで使用されていないことを確認します。
...
1. Ollamaをシステムにインストールします（[ollama.com](https://ollama.com/download)）
2. 必要なモデルを取得します（例：`ollama pull gemma4:e4b`）
...
**A**: できます。各ブラウザタブは独立した会話セッションなので、複数のタブを開いて別々のアプリやトピックを並行して進められます。会話を保存・エクスポートして後から再開することも可能です。
