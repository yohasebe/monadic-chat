...
- **Edit**<br />チャットインターフェイス上で直接メッセージをインライン編集できます。クリックすると、メッセージの代わりにテキストエリアが表示され、「保存」と「キャンセル」ボタンが付いています。メッセージを編集して変更を保存するか、キャンセルして元のメッセージに戻すことができます。編集されたメッセージは会話内の元の位置を維持し、後続のメッセージも保持されます。
