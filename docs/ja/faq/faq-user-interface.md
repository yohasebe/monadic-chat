...
**A**: トークン数は自動的に計算されます。表示される値は近似値であり、使用しているモデルによって若干異なる場合があります。
...
##### Q: Stopをクリックした後、Web UIに「Connecting…」や「Connection lost」が一時的に表示されます。問題がありますか？ :id=stop-connecting-flicker

**A**: 問題ありません。Monadic Chatを意図的に停止すると、組み込みブラウザでは再接続の試行が抑制され、コンソールのステータスにはシャットダウン中「Stopped」と表示されます。外部ブラウザでタブを開いたままにしていた場合は再接続を試み続けることがありますが、次回のStart後にリロードすれば元に戻ります。
