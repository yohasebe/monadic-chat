...
- **OpenAIモデル**: 「whisper-1」、「gpt-4o-mini-transcribe」、「gpt-4o-transcribe」、「gpt-4o-transcribe-diarize」。新しいモデルは一般的に精度と文字起こし品質が向上しています。diarizeモデルは複数人の音声における話者識別をサポートし、各話者の発言にラベルを付けます。
...
**音声設定**パネルの**音声認識モデル**ドロップダウンで希望のモデルを選択できます。利用可能なモデルは設定されたAPIキーに基づいて自動的に有効化されます。OpenAIモデルにはOpenAI APIキーが必要で、GeminiモデルにはGemini APIキーが必要です。選択内容はブラウザに保存され、セッションをまたいで保持されます。Monadic Chatは選択したモデルに基づいて音声フォーマットと処理を自動的に最適化します。言語設定は期待される言語のヒントとして機能しますが、話された言語を柔軟に認識します。
