...
| コンテナ | ポリシー | 理由 |
|----------|----------|------|
| Ruby | `always` | メインアプリサーバー — クラッシュ時に自動復旧 |
| Python | デフォルト（`no`） | オンデマンド起動。独立したライフサイクルなし |
| PGVector | デフォルト（`no`） | Monadic専用DB。Docker Resource Saverの阻害を防止 |
| Selenium | デフォルト（`no`） | オンデマンド起動。独立したライフサイクルなし |
