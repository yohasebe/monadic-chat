...
2. **jQuery依存なし**: jQuery はコードベースから完全に除去済み（2026-03-30）。全DOM操作はバニラJS
3. **イベント処理**: 実際のDOMイベントが自然に伝播（`{bubbles: true}` を使用）
...
3. **jQuery除去**: 全 `$()` 呼び出しを `document.getElementById()`, `querySelector()` 等に置換
4. **フィクスチャの読み込み**: ESMモジュール問題を避けるために簡素化
5. **ClipboardEvent**: カスタムclipboardDataプロパティ付きEventを使用

### モックベーステストとの主な違い
- 手動モックメンテナンスが不要（jQueryモック排除済み）
- テストはバニラDOM APIによる実際のユーザー動作を反映
