...
| 50 | `math` | math有効 | LaTeXフォーマット指示を追加 |
...
**重要**: バックスラッシュを含むプロンプト定数（例：LaTeX）を定義する場合、シングルクォートのヒアドキュメントを使用してください：
...
- `websocket.rb`：KaTeX用の追加インジェクション（約45行）
...
数式レンダリングは異なるモードで異なるエスケープを必要とします：
