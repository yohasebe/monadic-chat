...
| Mid-conversation tool changes | beta_header | Always on (provider-wide) | `claude_helper.rb` (`build_claude_headers_and_body`) | Beta: `mid-conversation-tool-changes-2026-07-01`. Keeps the prompt cache alive when progressive tool disclosure rewrites `tools` between turns; without it the whole cached prefix is re-created on that turn. Verified 2026-07-25 on every catalog model (Sonnet 5: `cache_creation` 4912 → 0, `cache_read` 0 → 4912). Anthropic documents Opus 5 / Opus 4.8 / Fable 5 only; unsupported models ignore the header rather than erroring, so narrowed coverage would silently revert to the old behavior |
| Forced tool choice avoidance | model_param | Model-gated (`rejects_forced_tool_choice`) | `claude_helper.rb` (`configure_claude_tools`) | Claude Fable 5.1 returns 400 on `tool_choice` `any`/`tool`; the helper sends `auto` and relies on the prompt to steer tool use |
| Thinking block binding (`drop_block`) | beta_header + model_param | Model-gated (`thinking_block_binding`) | `claude_helper.rb` (`build_claude_headers_and_body`) | Beta: `thinking-binding-controls-2026-08-01` + `thinking.block_binding.prefix_mismatch_behavior: drop_block`. Fable 5.1 binds thinking blocks to their prefix; a `tools` rewrite mid tool-loop would otherwise be a 400 on accounts created on or after 2026-08-31. Drops are logged from `input_transformations` |
