...
describe('Cards Module', () => {
  ...
  beforeEach(() => {
    ...
    testEnv = setupTestEnvironment({
      ...
      setupAppFunctions: () => {
        ...
        // Setup card creation utility that returns a DOM element with mock structure
        global.createCard = jest.fn((role, badge, html, lang = "en", mid = "", status = true, images = []) => {
          ...
          // Set innerHTML with card-text wrapper
          card.innerHTML = `<div class="card-text">${htmlContent}</div>`;
  ...
  describe('Card Creation', () => {
    it('creates user cards with proper structure', () => {
      ...
      expect(card.querySelector('.role-user')).not.toBeNull();
      expect(card.querySelector('.card-text').innerHTML).toContain('Hello world');
    ...
    it('creates assistant cards with proper structure', () => {
      ...
      expect(card.querySelector('.role-assistant')).not.toBeNull();
      expect(card.querySelector('.card-text').innerHTML).toContain('I am an assistant');
    });

    it('escapes HTML in system cards', () => {
      const card = global.createCard('system', '<i class="icon"></i>', '<b>System message</b>');

      expect(card.querySelector('.role-system')).not.toBeNull();
      expect(global.escapeHtml).toHaveBeenCalledWith('<b>System message</b>');
      expect(card.querySelector('.card-text').innerHTML).toContain('&lt;b&gt;System message&lt;/b&gt;');
    });
    ...
    it('preserves line breaks in system cards', () => {
      ...
      global.createCard = (role, badge, html, lang = "en", mid = "", status = true, images = []) => {
        ...
        const card = document.createElement('div');
        card.className = 'card';
        card.innerHTML = `<div class="card-text"><div class="role-${role}">${processedHtml}</div></div>`;
        return card;
      };

      const card = global.createCard('system', '<i class="icon"></i>', 'Line 1\nLine 2\nLine 3');

      expect(card.querySelector('.card-text').innerHTML).toContain('Line 1<br>Line 2<br>Line 3');
    ...
    it('adds images to cards when provided', () => {
      ...
      expect(card.querySelector('.card-text').innerHTML).toContain('base64-image');
      expect(card.querySelector('.card-text').innerHTML).toContain('Test Image');
    });
    ...
    it('handles PDF attachments correctly', () => {
      ...
      expect(card.querySelector('.card-text').innerHTML).toContain('pdf-preview');
      expect(card.querySelector('.card-text').innerHTML).toContain('Document.pdf');
    });
